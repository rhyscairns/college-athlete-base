...
interface PlayerScholarshipsPageProps {
    params: Promise<{ playerId: string }>;
}

export async function generateMetadata({ params }: PlayerScholarshipsPageProps): Promise<Metadata> {
    const { playerId } = await params;
    return {
        title: 'Scholarship Offers',
        description: `Scholarship offers received by player ${playerId}`,
    };
}

/**
 * Player Scholarship Offers List Page
 *
 * Protected server component — requires a valid player session.
 * Fetches all scholarship offers received by the player and renders ScholarshipsTable.
 *
 * Auth: JWT cookie validated via verifyToken; redirects to /login on failure.
 */
export default async function PlayerScholarshipsPage({ params }: PlayerScholarshipsPageProps) {
    const { playerId } = await params;
    ...
    try {
        ...
        logger.info('Player scholarships page loaded', { playerId, count: scholarships.length });
    } catch (error) {
        logger.error('Failed to fetch scholarships for player page', { playerId }, error instanceof Error ? error : new Error('Unknown error'));
        scholarships = [];
    }
            ...
            <div className="max-w-6xl mx-auto px-4 py-8">
                ...
                <header className="relative overflow-hidden text-center px-6 pt-12 pb-10 mb-8 rounded-2xl">
                    ...
                    <p className="text-base" style={{ color: 'var(--text-mid)' }}>
                        Scholarship offers you&apos;ve received from coaches
                    </p>
                </header>
