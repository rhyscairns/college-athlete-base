...
interface CoachEarningsPageProps {
...
async function getEarningsData(coachId: string): Promise<EarningsData | null> {
    try {
        ...
        console.error('[CoachEarningsPage] getEarningsData failed:', error);
        return null;
    }
}

/**
 * Coach Earnings Overview Page
 * Server component — queries DB directly and renders EarningsOverview.
 * Requirements: 4.1, 4.2, 4.3, 4.4, 4.5
 */
export default async function CoachEarningsPage({ params }: CoachEarningsPageProps) {
    ...
    return (
        <div className="p-4 sm:p-6 max-w-4xl mx-auto">
            <EarningsOverview data={data} basePath={`/coach/${coachId}/earnings`} />
        </div>
    );
}
